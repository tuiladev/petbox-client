import { images } from '~/assets'
import Logo from '~/components/common/Logo'

const AuthBanner = () => {
  return (
    <div className='relative hidden w-full max-w-2/5 overflow-hidden rounded-2xl shadow-lg md:block md:my-18! lg:max-w-lg'>
      <div className='absolute top-12 left-1/2 z-10 w-9/10 -translate-x-1/2'>
        <Logo type='dark' className='mx-auto mb-9 max-w-2xs!' />
        <h2 className='title-2xl text-center leading-normal! text-white'>
          "Mỗi thú cưng đều xứng đáng với những điều tuyệt vời !"
        </h2>
      </div>
      {/* Backgound images */}
      <img
        src={images.auth_banner}
        alt='Banner'
        className='h-full w-full object-cover object-center'
      />
    </div>
  )
}

export default AuthBanner
